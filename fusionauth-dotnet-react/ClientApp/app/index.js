...
class App extends React.Component {
  ...
  componentDidMount() {
    ...
    fetch(`${config.baseUrl}/api/WeatherForecast`, {
      credentials: "include", // fetch won't send cookies unless you set credentials
    })
      .then((response) => response.json())
      .then((response) =>
    ...
    return (
      <div id="App">
        <header>
          ...
          <LogInOut body={this.state.body} uri={`${config.baseUrl}`} />
        </header>
        <main>
          <UserData
          ...
          />
          <Response body={this.state.body} title="FusionAuth Response" />
          <Response body={this.state.weather} title="Weather Response" />
        </main>
        <footer>
